...
# MSE.
def fMSE (w, Xtilde, y):
    return cp.mean(((cp.dot(Xtilde.T, w)) - y)**2) / 2.0
...
def gradfMSE (w, Xtilde, y, alpha = 0.):
    w_reg_gpu = copy(w)
    w_reg_gpu[-1,0] = 0
    # w_reg_gpu = cp.asarray(w_reg)
    # w_gpu = cp.asarray(w)
    # y_gpu = cp.asarray(y)
    # Xtilde_gpu = cp.asarray(Xtilde)
    return (Xtilde.dot(Xtilde.transpose().dot(w) - y))/np.shape(Xtilde)[1] + (alpha/(2*np.shape(Xtilde)[1])) * w_reg_gpu.transpose().dot(w_reg_gpu)

# Given a design matrix Xtilde and labels y, train a linear regressor for Xtilde and y using the analytical solution.
def method1 (Xtilde, y):
    Xtilde_gpu = cp.asarray(Xtilde)
    y_gpu = cp.asarray(y)
    return cp.asnumpy(cp.linalg.solve(Xtilde_gpu.dot(Xtilde_gpu.transpose()),cp.eye(len(Xtilde_gpu))).dot(Xtilde_gpu).dot(y_gpu))
...
def gradientDescent (Xtilde, y, alpha = 0.):
    ...
    w = cp.random.randn(np.shape(Xtilde)[0], 1) * 0.01
    Xtilde_gpu = cp.asarray(Xtilde)
    y_gpu = cp.asarray(y)
    for i in tqdm(range(T)):
        grad = gradfMSE(w, Xtilde_gpu, y_gpu, alpha=alpha)
